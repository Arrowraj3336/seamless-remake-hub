...
export const Beam = forwardRef<any, BeamProps>(({ children, position, stride = 4, width = 8, ...props }, fRef) => {
  ...
  useFrame(() => {
    ...
    if (!streaks.current.instanceMatrix.updateRanges[0]) {
      (streaks.current.instanceMatrix as any).updateRanges[0] = { start: 0, count: 0 }
    }
    (streaks.current.instanceMatrix as any).updateRanges[0].count = range * 16
    ...
    if (!glow.current.instanceMatrix.updateRanges[0]) {
      (glow.current.instanceMatrix as any).updateRanges[0] = { start: 0, count: 0 }
    }
    (glow.current.instanceMatrix as any).updateRanges[0].count = range * 16
