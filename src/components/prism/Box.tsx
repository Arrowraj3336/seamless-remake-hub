...
export const Box = forwardRef<THREE.Group, BoxProps>((props, ref) => {
  ...
  useFrame(() => {
    if (inner.current) {
      lerpC((inner.current.material as THREE.MeshStandardMaterial).emissive, hovered ? 'white' : '#454545', 0.1)
    }
  })
  
  return (
    <group scale={0.5} ref={ref} {...props}>
      {/* A low-res, invisible box for raycasting */}
      <mesh 
        visible={false} 
        geometry={boxGeometry}
        {...{ onRayOver: () => hover(true), onRayOut: () => hover(false) } as any}
      />
